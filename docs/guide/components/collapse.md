...
onMounted(()=>{
    ...
    // })
     import('../../../packages/collapse/Collapse.vue').then(module => {
      mapComponent.value = module.default
    })
...
<br/>
<component v-if="mapComponent" :is="mapComponent" :collapse-data="collapseData" v-model:active-key="key"></component>
...
<br/>
<component v-if="mapComponent" :is="mapComponent" :collapse-data="collapseData" v-model:active-key="activeKey">
 <template #header="{ header, key }">
    <span v-if="key==='1'" style="color: pink;">burlywood color: {{ header }} (key = {{ key }})</span>
  </template>
   <template #lang>typescript</template>
  <template #text="{ text, key }">
    <span v-if="key==='1'" style="color: pink;">burlywood color: {{ text }} (key = {{ key }})</span>
  </template>
  </component>
